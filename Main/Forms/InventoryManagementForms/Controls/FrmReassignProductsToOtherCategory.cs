...
{
    public partial class FrmReassignProductsToOtherCategory : Form
    {
        ...
        private void FrmReassignProductsToOtherCategory_Load(object sender, EventArgs e)
        {
            ...
            var categories = _productCategoryData.GetAllNotDeleted().Where(x => x.Id != CategoryIdToDelete).ToList();
            ...
            var categoryToDeleteDetails = _productCategoryData.Get(CategoryIdToDelete);

            if (categoryToDeleteDetails != null)
            {
                this.LblCategoryToDelete.Text = categoryToDeleteDetails.ProdCategory;
            }
        }
